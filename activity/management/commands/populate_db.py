...
from docent.settings import YELP_API_KEY, TICK_API_KEY, HP_API_KEY
...
class Command(BaseCommand):
    ...
    def _pull_json(self, search_term):
        search_results = yelp_api.search_query(term=search_term, location='reno, nv', sort_by='rating', limit=50)
    ...
    def _create_ticketmaster_activities(self):
        data = requests.get('https://app.ticketmaster.com/discovery/v2/events.json?stateCode=NV&city=Reno&size=100&apikey='+TICK_API_KEY).json()
        events = data['_embedded']['events']
        for e in events:
            activity = Activity(
                name=e['name'],
    ...
    def _create_hiking_activities(self):
        data = requests.get('https://www.hikingproject.com/data/get-trails?lat=39.5296&lon=-119.8138&maxResults=100&key='+HP_API_KEY).json()
        trails = data['trails']
        for trail in trails:
            activity = Activity(
                name=trail['name'],
